...
if($args->exportType=='XLS'){
		if($exporting_just_one_tc){
			...
			$conn=connectToDB();
			$content .= $tsuite_mgr->$pfn($args->container_id,$args->tproject_id,$conn,$optExport,$conn);
			...
			$testcase=array();
			$size=count($testsuiteData);
			for($i=0;$i<$size;$i++){
				$testcase[$i]=$tcase_mgr->getTestCase($testsuiteData[$i]['id'],$conn);
			}
			/*$innerSize=count($testcase[0]['name']);
			for($i=0;$i<$innerSize;$i++){
				echo $testcase[0]['name'][$i];
			}*/
			//exit();
			$tcversions=array();
			$steps=array();
			...
			for($j=0;$j<$size;$j++){
				$i=0;
				foreach ($testcase[$j]['id'] as $key => $value) {
					$tcversions[$value]=$tcase_mgr->getTCVersion($value,$conn);
					$steps[$value]=$tcase_mgr->getSteps($tcversions[$value]['id'],$conn);	
				}
			}
			...
			for($j=0;$j<$size;$j++){
			foreach ($testcase[$j]['id'] as $key => $value) {
				$innerSize=count($steps[$value]['id']);
				$actions[$value]='';
				$expected_results[$value]='';
				for($i=0;$i<$innerSize;$i++){
					$actions[$value].=$steps[$value]['actions'][$i];
					$expected_results[$value].=$steps[$value]['expected_results'][$i];
				}
			}
			}
			
			//downloadXLSFile($testsuiteData,$testcase,$tcversions,$actions,$expected_results);
			downloadXLSFile($testsuiteData,$testcase,$tcversions,$steps,$gui->export_filename);
...
$gui->exportTypes=array('XML' => 'XML','XLS' => 'XLS');
