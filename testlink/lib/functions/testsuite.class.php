...
class testsuite extends tlObjectWithAttachments
{
  ...
  */
  ...
  //Aameer Ausekar
	function exportTestSuiteDataToXLS($container_id,$tproject_id,&$conn,$optExport = array(),$count=1)
	{
		$sql='select id,name from nodes_hierarchy where id='.$container_id;
		$result=$conn->query($sql);
		while($row = $result->fetch_assoc()) 
		{
			$xlsTC='|'.$row['id'].'|'.$row['name'];
		}
		$sql='select id,name from nodes_hierarchy where parent_id='.$container_id.' and node_type_id=2';
		$result=$conn->query($sql);
